<template>
  <div class="Health" id="health">
    <v-btn block color="gray" v-on:click.stop="dialog = true">Health and Physical Education</v-btn>
    <v-layout row justify-center>
      <v-dialog v-show="dialog"  v-model="dialog" fullscreen transition="dialog-bottom-transition">
        <v-card>
          <v-toolbar dark class="blue">
            <v-btn icon @click.native="dialog = false" dark>
              <v-icon>close</v-icon>
            </v-btn>
            <v-toolbar-title>Health and Physical Education</v-toolbar-title>
            <v-spacer></v-spacer>
            <v-toolbar-items>
            </v-toolbar-items>
          </v-toolbar>
          <v-expansion-panel expand>
            <v-expansion-panel-content :class="course.CURRENT_ENRL/course.MAX_ENRL >= .85 ? 'red lighten-4' : 'grey lighten-3'" v-for="course in healthClasses" :key="course.id">
...
export default {
        name: 'health',
        props: ['healthClasses', 'randomValues', 'convertToRange'],
        components: {
          modal,
        },
        data() {
